...
class coinbase():
    ...
    def __init__(self):
        self.websocket = websockets.connect('wss://ws-feed.pro.coinbase.com')

    def subscr(self, chan):
        msg = {
            "type": "subscribe",
            "channels": chan
        }
...
chn = [{"name": "ticker", "product_ids": ["XRP-USD"] }]
msgStreamBitmex = coinbase().subscr(chn)

try:
    loop = asyncio.get_event_loop()
    loop.create_task(coinbase().pubStream(msgStreamBitmex))
    loop.run_forever()
