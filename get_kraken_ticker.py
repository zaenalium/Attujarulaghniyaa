...
class kraken():
    ...
    def __init__(self):
        self.websocket = websockets.connect('wss://ws.kraken.com')

    def subscr(self, chn, type):
        msg = {
              "event": "subscribe",
              "pair": chn,
              "subscription": {
    ...
    async def pubStream(self, msg):
        ...
        async with self.websocket as websocket:  # create the connexion
            ...
            try:
                ...
                print('KRAKEN Public WS down. Restarting : ', str(e))
...
chn = ['BTC/USD']
type = 'ohlc'
msgStreamBitmex = kraken().subscr(chn, type)

try:
    loop = asyncio.get_event_loop()
    loop.create_task(kraken().pubStream(msgStreamBitmex))
    loop.run_forever()
