...
import asyncpg
engine = sa.create_engine('postgresql://postgres:T0raja$am@localhost:5432/baitulbayanat')
...
async def get_rest_indodax(method,symb, tbl):
    con = await asyncpg.connect('postgresql://postgres:T0raja$am@localhost:5432/baitulbayanat')
    while True:
        try:
